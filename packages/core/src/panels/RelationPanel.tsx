...
const CARDINALITY_OPTIONS: { value: Cardinality; label: string; symbol: string }[] = [
  { value: 'EXACTLY_ONE', label: 'Exactly One', symbol: '||' },
  { value: 'ZERO_OR_ONE', label: 'Zero or One', symbol: '|o' },
  { value: 'ONE_OR_MORE', label: 'One or More', symbol: '|{' },
  { value: 'ZERO_OR_MORE', label: 'Zero or More', symbol: 'o{' },
];
    ...
    <div className="side-panel__content">
      ...
      <div className="side-panel__section px-4 py-3 border-b border-slate-100">
        <div className="flex items-center justify-between text-sm">
          <span className="font-medium text-slate-600">{sourceEntity?.name || 'Unknown'}</span>
          <span className="text-slate-400">→</span>
          <span className="font-medium text-slate-600">{targetEntity?.name || 'Unknown'}</span>
        </div>
      </div>
      ...
      <div className="side-panel__section px-4 py-3 border-b border-slate-100">
        <label className="side-panel__label block text-sm font-medium text-slate-600 mb-1">
          Source Cardinality ({sourceEntity?.name})
        </label>
        <select
      ...
      <div className="side-panel__section px-4 py-3 border-b border-slate-100">
        <label className="side-panel__label block text-sm font-medium text-slate-600 mb-1">
          Target Cardinality ({targetEntity?.name})
        </label>
        <select
      ...
      <div className="side-panel__section px-4 py-3 border-b border-slate-100">
        ...
        <p className="text-xs text-slate-400 mt-1">
          Uncheck for non-identifying (dashed line)
        </p>
      </div>
