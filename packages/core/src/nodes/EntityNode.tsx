...
const AttributeRow = memo(
  ({
    ...
    const isCommonType = COMMON_ATTRIBUTE_TYPES.includes(
      attr.type as (typeof COMMON_ATTRIBUTE_TYPES)[number]
    );
    const [isEditingName, setIsEditingName] = useState(false);
    const [isEditingType, setIsEditingType] = useState(false);
    const [isCustomMode, setIsCustomMode] = useState(
      !isCommonType && attr.type !== ''
    );
    ...
    return (
      <div className="entity-node__attribute px-3 py-1.5 border-b border-slate-100 last:border-b-0 text-sm flex items-center gap-2">
        {icon && (
          <span className="entity-node__attribute-icon w-4">{icon}</span>
        )}
  ...
  ({ data, selected }: NodeProps<EntityNodeData>) => {
    ...
    return (
      ...
      >
        ...
        <div
          onDoubleClick={handleHandleDoubleClick('top')}
          className="absolute -top-2 left-1/2 -translate-x-1/2 p-1 cursor-pointer"
        >
          <Handle
            type="source"
        ...
        <div
          onDoubleClick={handleHandleDoubleClick('right')}
        ...
        <div
          onDoubleClick={handleHandleDoubleClick('bottom')}
          className="absolute -bottom-2 left-1/2 -translate-x-1/2 p-1 cursor-pointer"
        >
          <Handle
            type="source"
        ...
        <div
          onDoubleClick={handleHandleDoubleClick('left')}
          className="absolute top-1/2 -left-2 -translate-y-1/2 p-1 cursor-pointer"
        >
          <Handle
            type="source"
